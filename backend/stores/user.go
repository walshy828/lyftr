...
const userSettingsSelect = `SELECT user_id, weight_unit, calorie_target, protein_target, carb_target, fat_target, cholesterol_target, sodium_target, food_allergies, food_dislikes, food_likes, plan_history_start, ai_health_insights_opt_in FROM user_settings`
...
func (s *UserStore) UpsertSettings(uid int64, req models.UpdateSettingsRequest) (models.UserSettings, error) {
	...
	var st models.UserSettings
	err := s.db.QueryRow(
		`INSERT INTO user_settings (user_id, weight_unit, calorie_target, protein_target, carb_target, fat_target, cholesterol_target, sodium_target, food_allergies, food_dislikes, food_likes, plan_history_start, ai_health_insights_opt_in)
		 VALUES (?, COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?), COALESCE(?, ?))
		 ...
		 RETURNING user_id, weight_unit, calorie_target, protein_target, carb_target, fat_target, cholesterol_target, sodium_target, food_allergies, food_dislikes, food_likes, plan_history_start, ai_health_insights_opt_in`,
		uid,
		...
		req.PlanHistoryStart, req.AIHealthInsightsOptIn,
	).Scan(&st.UserID, &st.WeightUnit, &st.CalorieTarget, &st.ProteinTarget, &st.CarbTarget, &st.FatTarget,
		&st.CholesterolTarget, &st.SodiumTarget, &st.FoodAllergies, &st.FoodDislikes, &st.FoodLikes, &st.PlanHistoryStart,
		&st.AIHealthInsightsOptIn)
	if err != nil {

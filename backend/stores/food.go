...
const foodLogSelect = `SELECT id, user_id, name, brand, meal, calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol, servings, serving_size, serving_size_grams, serving_size_ml, barcode, image_url, source, logged_at, created_at FROM food_logs`
...
func scanFoodLog(row interface{ Scan(...any) error }, f *models.FoodLog) error {
	return row.Scan(
		...
		&f.Servings, &f.ServingSize, &f.ServingSizeGrams, &f.ServingSizeML, &f.Barcode, &f.ImageURL, &f.Source,
...
func (s *FoodStore) Create(uid int64, req models.LogFoodRequest) (models.FoodLog, error) {
	res, err := s.db.Exec(
		...
		req.Servings, req.ServingSize, req.ServingSizeGrams, req.ServingSizeML, req.Barcode, req.ImageURL, req.Source, req.LoggedAt,
	)
	if err != nil {
...
func (s *FoodStore) Update(uid, id int64, req models.LogFoodRequest) (models.FoodLog, error) {
	res, err := s.db.Exec(
		`UPDATE food_logs SET name=?, brand=?, meal=?, calories=?, protein=?, carbs=?, fat=?, fiber=?, sugar=?, sodium=?, cholesterol=?,
		 servings=?, serving_size=?, serving_size_grams=?, serving_size_ml=?, barcode=?, image_url=?, source=?, logged_at=?
...
func (s *FoodStore) RecentFrequentFoods(uid int64, sinceDays, limit int) ([]models.RecentFood, error) {
	rows, err := s.db.Query(
		...
		`SELECT id, user_id, name, brand, meal, calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol, servings, serving_size, serving_size_grams, serving_size_ml, barcode, image_url, source, logged_at, created_at, COUNT(*) AS log_count, MAX(logged_at) AS last_logged
		 FROM food_logs
...
const savedFoodSelect = `SELECT id, user_id, name, brand, calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol, serving_size, serving_size_grams, serving_size_ml, barcode, image_url, created_at FROM saved_foods`
...
func scanSavedFood(row interface{ Scan(...any) error }, f *models.SavedFood) error {
	return row.Scan(&f.ID, &f.UserID, &f.Name, &f.Brand, &f.Calories, &f.Protein, &f.Carbs, &f.Fat,
		&f.Fiber, &f.Sugar, &f.Sodium, &f.Cholesterol, &f.ServingSize, &f.ServingSizeGrams, &f.ServingSizeML, &f.Barcode, &f.ImageURL, &f.CreatedAt)
}
...
func (s *FoodStore) CreateSaved(uid int64, req models.SaveFoodRequest) (models.SavedFood, error) {
	res, err := s.db.Exec(
		`INSERT INTO saved_foods (user_id, name, brand, calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol, serving_size, serving_size_grams, serving_size_ml, barcode, image_url)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		uid, req.Name, req.Brand, req.Calories, req.Protein, req.Carbs, req.Fat, req.Fiber, req.Sugar, req.Sodium, req.Cholesterol,
		req.ServingSize, req.ServingSizeGrams, req.ServingSizeML, req.Barcode, req.ImageURL,
	)
	if err != nil {
...
func (s *FoodStore) UpdateSaved(uid, id int64, req models.UpdateSavedFoodRequest) (models.SavedFood, error) {
	res, err := s.db.Exec(
		`UPDATE saved_foods SET name=?, brand=?, calories=?, protein=?, carbs=?, fat=?, fiber=?, sugar=?, sodium=?, cholesterol=?,
		 serving_size=?, serving_size_grams=?, serving_size_ml=?, barcode=?, image_url=?
		 WHERE id=? AND user_id=?`,

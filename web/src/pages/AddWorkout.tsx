...
export default function AddWorkout() {
  ...
  const loadFromProgram = (program: types.Program) => {
    ...
    const newExercises = (program.exercises || []).map(ex => {
      ...
      return { exercise_id: ex.exercise_id, notes: ex.notes || '', rest_seconds: ex.rest_seconds ?? (settings.rest_seconds_default ?? 90), sets: (ex.sets || []).map(s => ({ set_number: s.set_number, reps: s.target_reps, weight: s.target_weight, duration: s.target_duration_seconds })) }
    })
  ...
  const addExercise = (exercise: types.Exercise) => {
    ...
    const set = isTimed(exercise) ? { set_number: 1, reps: 0, weight: 0, duration: exercise.default_duration_seconds || 30 } : { set_number: 1, reps: 0, weight: 0 }
    setFormData(prev => ({ ...prev, exercises: [...prev.exercises, { exercise_id: exercise.id, notes: '', rest_seconds: settings.rest_seconds_default ?? 90, sets: [set] }] }))
    setShowPicker(false)
    setError('')
  }
                      ...
                      <div key={setIdx} className="flex gap-2 items-end bg-surface-raised/40 p-3 rounded-lg border border-surface-border/50">
                        ...
                        {isTimed(exercise) ? (
                          <div className="flex-1 min-w-0">
                            <label className="text-xs text-tx-muted font-medium uppercase tracking-wider block mb-1">Duration (sec)</label>
                            <input type="number" inputMode="numeric" value={set.duration || ''} onChange={e => updateSet(exIdx, setIdx, 'duration', e.target.value)} placeholder="30" className="input text-sm w-full" min="0" />
                          </div>
                        ) : (
                          <>
                            <div className="flex-1 min-w-0">
